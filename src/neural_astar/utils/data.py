...
class MazeDatasetWithFields(MazeDataset):
    """Distance fields를 포함하는 확장된 dataset"""
    ...
    def __init__(
        ...
        super().__init__(filename, split, pct1, pct2, pct3, num_starts)
        
        self.geodesic_fields = None
        self.obstacle_fields = None
        
        if precompute_fields:
            # Check cache
            cache_path = None
            if cache_dir:
                os.makedirs(cache_dir, exist_ok=True)
                dataset_name = os.path.basename(filename).replace('.npz', '')
                cache_path = f"{cache_dir}/{dataset_name}_{split}_fields.npz"
            
            if cache_path and os.path.exists(cache_path):
                print(f"Loading cached distance fields from {cache_path}")
                cached = np.load(cache_path)
                self.geodesic_fields = cached['geodesic']
                self.obstacle_fields = cached['obstacle']
            else:
                from .distance_field import precompute_distance_fields
                
                print(f"Precomputing distance fields for {split} set...")
                self.geodesic_fields, self.obstacle_fields = precompute_distance_fields(
                    self.map_designs,
                    self.goal_maps,
                    show_progress=True
                )
                ...
                # Save cache
                if cache_path:
                    print(f"Saving distance fields cache to {cache_path}")
                    np.savez_compressed(
                        cache_path,
                        geodesic=self.geodesic_fields,
                        obstacle=self.obstacle_fields
                    )
    
    def __getitem__(self, index: int):
        map_design, start_map, goal_map, opt_traj = super().__getitem__(index)
        
        geodesic_field = None
        obstacle_field = None
        
        if self.geodesic_fields is not None:
            geodesic_field = self.geodesic_fields[index]
        
        if self.obstacle_fields is not None:
            obstacle_field = self.obstacle_fields[index]
        
        return map_design, start_map, goal_map, opt_traj, geodesic_field, obstacle_field


def create_dataloader_with_fields(
    filename: str,
    split: str,
    batch_size: int,
    num_starts: int = 1,
    ...
    dataset = MazeDatasetWithFields(
        ...
        precompute_fields=precompute_fields,
        cache_dir=cache_dir
    )
